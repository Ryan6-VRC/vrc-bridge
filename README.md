...
**Which manifest applies is the avatar's own statement**: the entry declares an unsynced Int `QuantChannel/Manifest` whose *default value* is the manifest id, and the directory reads it over OSCQuery. Like the wardrobe, it never reads on the avatar change itself: the directory clears on the change and re-reads when a consumer next asks, retrying at most every 2 s until the loaded avatar answers. That is what carries it through the states where a read cannot succeed — no client discovered yet, a foreign OSCQuery peer holding the target, an avatar still loading — each of which it logs once and retries out of unaided.
