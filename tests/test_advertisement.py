...
def test_announcement_is_pinned_to_the_served_interface(monkeypatch):
    ...
    class RecordingZeroconf(Zeroconf):
        def __init__(self, *args, **kwargs):
            seen["interfaces"] = kwargs.get("interfaces", "<default: InterfaceChoice.All>")
            super().__init__(*args, **kwargs)

    monkeypatch.setattr(om, "Zeroconf", RecordingZeroconf)

    mgr = OSCManager(advertise=False)          # no register_service: that costs ~1.7s
    mgr.start()
    try:
        assert seen["interfaces"] == [mgr.host]
    finally:
        mgr.stop()
