...
The attribution is what is open, and the suspect is `start()`'s bare `Zeroconf()`: it defaults to `InterfaceChoice.All`, so we announce on every interface the host has, while the record itself carries a single loopback address. A client that opens a sender per announcement it receives would produce exactly two on a host whose loopback and LAN adapters both reach it — which is the observed signature, not a coincidence to wave at. Ruled out already: the service record is not itself duplicated, and there is only one candidate destination (one service, one address, `VRBridge.local.` resolving to `127.0.0.1` alone), so the missing `OSC_IP` in `?HOST_INFO` above is not implicated here.

The hypothesis makes a falsifiable prediction, and checking it costs nothing: **the sender count should track the number of interfaces the announcing host actually reaches the client on.** Count them on the machine the measurement was taken on — two senders on a many-interfaced host argues against per-announcement senders as much as two-on-two argues for it. Then two tests separate the cases: pin the interfaces to loopback and re-run the source-endpoint probe against a live client, and run a third-party OSCQuery receiver against the same client as a control. If pinning collapses it to one sender, this is a one-line fix in our own advertisement rather than a client behaviour to tolerate. Until then, step 7 must handle doubled inbound — every inbound-triggered callback runs twice, so a non-idempotent listener double-applies — but must not assume it is unfixable and reach for a dedupe window first. Inbound only: outbound sends appear once. One client build (`VRChat-Client-161618`), one machine.
...
**Pulse spacing is a real constraint, not a precaution.** VRChat applies the latest value per parameter per frame rather than queueing — a queue would fall permanently behind under face-tracking volume — so two writes inside one frame collapse to the later and a trailing 0 sharing a frame with the next value is lost silently. Measured end to end against a timestamped receiver, a stepped scroll separates its pulses by the configured gap to within half a millisecond, which is why `PULSE_GAP_SECS` is `1/20` s: `1/30` cleared a 30 fps frame by under a millisecond, which is not a margin.
