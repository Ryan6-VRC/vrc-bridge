...
- **Also proven headlessly** (`tests/test_target_selection.py`): target *selection* — which discovered service wins the slot, that only a strictly better rank unseats an incumbent, that a republication of the current service is followed to its new port, that our own advertisement is never targeted, and that a named target refuses all four rules. Selection separates from discovery because `_consider_service` takes a name and a `ServiceInfo` and decides, so handing it one directly needs no network.
...
Everything above assumes a peer that does both halves of the handshake: announces itself over mDNS so `_consider_service` can rank it, and reads our `/?HOST_INFO` so it learns the port we floated. Lyuma's Av3Emulator does neither — it carries VRChat's OSC surface with no service discovery of any kind, listens on 9000, and sends to a fixed `127.0.0.1:9001`. Such a peer is reached by naming its ports instead: `target=(host, port)` for the send side and `bind_port=` for the receive side, from `OSCManager`, through `VRBridge`, out to `--osc-host`/`--osc-port`/`--osc-bind-port`.
...
**A named target suppresses discovery-driven retargeting entirely; discovery keeps observing and stops deciding.** `_consider_service` returns at its top, so nothing found can revise a target that was given, while `_BrowserListener` still records what it sees and `is_service_running` — which `osc_vrcft` reads for VRCFaceTracking's presence — answers exactly as before. The alternative, letting a named target be an initial pick that ranking may still unseat, fails on the case this exists for: VRChat scores 3 and the emulator would be pinned on loopback beside it, so a run aimed at the emulator would move onto the live avatar mid-session on mDNS callback timing. Ranking is for choosing among peers nobody named; naming one takes the question away rather than entering it as a bid. A pin also leaves `_current_service_name` at `None`, which is what makes `remove_service` structurally unable to clear a target no service backs.
