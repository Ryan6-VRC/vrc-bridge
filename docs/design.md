...
One gap is the client's rather than ours. **VRChat delivers every inbound message twice, from two UDP sender sockets it opens per discovered service.** Measured with a probe recording source endpoints: 8,024 datagrams over 20 s from exactly two endpoints, 4,012 each, and of 1,107 distinct payloads every one arrived twice with **zero** duplicated from the same endpoint. Different source ports mean two genuine sends, which is what rules out socket-layer duplication and a doubled handler alike — so reach for the source endpoint, not the counts, if this is ever re-tested. It sits upstream of us, the pre-audit install inherits it through the same discovery path, and it is **inbound only**: outbound sends appear once. Every inbound-triggered callback therefore runs twice, and a non-idempotent listener double-applies — whether to dedupe on `(address, args)` within a short window or to require idempotent handlers is step 7's call. Measured on one client build (`VRChat-Client-161618`) on one machine; confirm against a second install before treating it as universal.
