...
- **Also proven headlessly** (`tests/test_target_selection.py`): target *selection* — which discovered service wins the slot, that only a strictly better rank unseats an incumbent, that a republication of the current service is followed to its new port, that our own advertisement is never targeted, and that a named target is neither unseated by a rank-3 VRChat bid nor cleared by a service removal. Selection separates from discovery because `_consider_service` takes a name and a `ServiceInfo` and decides, so handing it one directly needs no network.
...
## Addressing a peer that does not advertise

Everything above assumes a peer that does both halves of the handshake: announces itself over mDNS so `_consider_service` can rank it, and reads our `/?HOST_INFO` so it learns the port we floated. Lyuma's Av3Emulator does neither — it carries VRChat's OSC surface with no service discovery of any kind, listens on 9000, and sends to a fixed `127.0.0.1:9001`. Such a peer is reached by naming its ports instead: `target=(host, port)` for the send side and `bind_port=` for the receive side, from `OSCManager`, through `VRBridge`, out to `--osc-host`/`--osc-port`/`--osc-bind-port`. The host names a send destination and nothing else — `OSCManager.host`, which the listener, the served tree and the mDNS interface pin all follow, stays at loopback — so a peer named off this machine can be sent to and cannot answer.

**Both, or the loop is one-directional.** The send override alone is the tempting half and it is not enough: a peer that cannot read `/?HOST_INFO` cannot learn a floating inbound port, so it goes on sending to the fixed one it was built with and every reply lands nowhere. The receive half is what the emulator's 9001 needs, and it introduces the only failure mode the port-0 bind never had — an occupied port — which `start()` reports naming both the port and the option that asked for it.

**A named target suppresses discovery-driven retargeting entirely; discovery keeps observing and stops deciding.** `_consider_service` returns at its top, so nothing found can revise a target that was given, while `_BrowserListener` still records what it sees and `is_service_running` — which `osc_vrcft` reads for VRCFaceTracking's presence — answers exactly as before.

**That governs the send side, and only the send side.** A pin does not touch `advertise`, so a pinned session still registers its mDNS service and still answers `/?HOST_INFO` — with the fixed bind port, if one was given. A live VRChat therefore finds it and pushes avatar params into the same cache and listener the named peer writes to, and an inbound value is whatever reached the bind port rather than necessarily the peer we aimed at. `advertise=False` closes it, which is why the two stay separate arguments; the CLI deliberately has no flag for it. The alternative, letting a named target be an initial pick that ranking may still unseat, fails on the case this exists for: VRChat scores 3 and the emulator would be pinned on loopback beside it, so a run aimed at the emulator would move onto the live avatar mid-session on mDNS callback timing. Ranking is for choosing among peers nobody named; naming one takes the question away rather than entering it as a bid. A pin also leaves `_current_service_name` at `None`, which is what makes `remove_service` structurally unable to clear a target no service backs.

Neither is a `settings.py` value. That file holds mapping and hardware feel; which peer to address is runtime wiring, like `--no-steamvr`.
...
## The wardrobe, and why it reads a marker off the avatar

`osc_wardrobe` swaps the worn avatar from a button on the wearer's own menu. It is a **wearer-facing feature**, and that is the only thing its machinery is priced against: this workspace's own probe-swapping loop is not the warrant and needs none of it — one `send("/avatar/change", "avtr_…")` on the library path does that, and §Settled decisions rules this repo is not the workspace's verification surface. Judge any addition here against the menu feature alone.

**Inbound `/avatar/change` is supported, and only the patch notes say so.** VRChat 2025.1.2 introduced it — send the address an avatar id as its argument — restricted to avatars in your favorites, recents, or your own uploads; 2025.4.2 then fixed it to match that documentation, having shipped working for favorites alone. **The reference pages contradict this by omission**: `docs/osc-avatar-parameters` and the community wiki describe only the outbound emission, so a reader who checks the reference concludes the direction does not exist. Do not "correct" this section from those pages.

**The emulator cannot round-trip it.** `/avatar/change` occurs once in `lyuma.av3emulator`, inside `GetOSCDataInto` — the outbound builder — with no inbound handler. The swap is therefore client-only, while everything about the avatar-side prefab is emulator-testable; `verify.md` splits those tiers, and its exemption for parameter traffic does not reach a client command.

**Why a marker parameter rather than one configured list.** The slot→avatar table cannot live on the avatar (they are a user's own avatar ids) and the bridge cannot otherwise know which table a given avatar's menu means. So each manifest carries an `id`, the avatar carries that number as the *default value* of an unsynced Int parameter, and the bridge reads it over OSCQuery. That is what lets two avatars carry different wardrobes, and what separates "no wardrobe here" from "wardrobe at rest". Ids are 1–255, which is not our choice: **Modular Avatar's inspector clamps an Int default to 0–255 and re-clamps whenever the sync type is touched**, so a larger id cannot be authored onto the avatar that would have to carry it. That bounds any id convention built on this mechanism — worth knowing before designing another one around a wider range.

**An avatar change invalidates the manifest, and re-arming needs a discriminator.** `osc.md` guarantees only that the tree 404s an address *no worn avatar declares* — a per-address property, not a per-transition one — and this mapping's premise is that several avatars declare this one at different values, so a read taken mid-transition can return the **outgoing** avatar's id and be indistinguishable from a correct answer. Adopting it would index the wrong table and swap somewhere the wearer never asked for, silently. Invalidating on the change is what makes the window safe: until something is adopted, a press does nothing.

Two discriminators re-arm it, and they are not equally strong. **An observed 404 proves teardown**, so the first value after one is necessarily the new avatar's. Failing that, two reads a configured gap apart that agree are accepted — which establishes that the value is *stable*, not that it is *current*, so it is a timing bet and not a proof. If the client turns out to serve the outgoing marker until replacement rather than for a bounded interval, no gap value rescues the second rule and only the 404 rule holds; that is the question a live probe should settle first, because a "persistent" answer invalidates the mechanism rather than retuning it. Which discriminator fired is logged for exactly that reason.

**A 404 must never end the read schedule.** During a swap the new avatar's node is not published yet, so the first read of a perfectly good wardrobe 404s — concluding "no wardrobe" there leaves a wardrobed avatar unarmed until the *next* avatar change, reachable only through the menu this feature exists to replace. Conclude that only when 404 persists across the whole budget.

**A read carries the generation it began under and refuses to commit if the world moved on.** Two avatar changes inside one read window — an ordinary scroll through favourites — otherwise let the finishing read arm the wardrobe with a table for an avatar nobody is wearing. Coalescing by "one request at a time" is what produced that: a superseding request must replace the in-flight one, never be dropped.

**The `/avatar/change` handler is registered ungated, and that is load-bearing.** It is the only trigger that re-reads the marker, so gating it would make a single failed read terminal until the process restarted — and since a *successful* swap is what triggers the read, the gated version fails precisely on working while an ineligible id leaves it healthy.

**Which is only safe because arming and enablement are separate.** `_active` is the arm state and the mapping never writes its own `enabled`; that belongs to the router. Conflating them let the ungated handler re-enable a mapping a router had deliberately switched off — a mapping that promotes itself is the one thing an ungated handler must not be able to do. It also cost a fail-loud guarantee: while the mapping disabled itself between a change and a successful read, `_gate` swallowed every press *before* the "no manifest is active" warning, so the whole re-arm window was silent. No other mapping in this repo calls its own `activate()`.

**There is no echo watchdog, and there cannot be one — the channel carries no outcome.** Measured against a live client (2026-08-04, one client build): VRChat echoes `/avatar/change` carrying **the id we sent** within 5 ms, does so identically for an ineligible id *and for a malformed one* (`avtr_not-a-uuid` acked in 2 ms), and **never emits again** when the avatar really loads. The echo is an acknowledgement of the request, not a report of the outcome, so nothing built on it could separate a working swap from a rejected one. Do not re-propose a watchdog on this address; anything needing the outcome must read it elsewhere. The worn avatar *is* observable on the account's profile page, which is the right instrument for a probe and firmly out of scope for this repo.

**Which is why there is no "already wearing it, skip the send" check.** The only thing that could say what is worn is that same echo, so a suppression built on it would, after a swap the client declined, believe we were wearing the avatar that failed — and then silently swallow the wearer's retry of that very slot. A button that stops working the first time it fails is far worse than a redundant send the client no-ops. The loop defence never depended on this: it is that the source is momentary and zero never swaps.

**The swap is accepted with the game menu open** (measured the same session), so the wearer does not have to close the menu they pressed the button from — which is the only way they could have pressed it.

**One consequence for the read budget.** Because the echo fires on the *request* rather than the load, the marker re-read schedule begins before the new avatar exists, and its whole budget (`settle_delay_secs` plus `max_reads × stable_gap_secs`) has to outlast a real avatar load — a cold download, not a cached swap. The shipped defaults give about 6.5 s. Nothing measures a worst-case load, so treat that as a floor rather than a considered value; the 404-tolerant schedule is what keeps an over-run merely slow instead of wrong.
