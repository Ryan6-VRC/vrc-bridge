...
**An avatar change invalidates the manifest, and re-arming needs two agreeing reads.** `osc.md` guarantees only that the tree 404s an address *no worn avatar declares* — and this mapping's premise is that several avatars declare this one at different values, so a read taken mid-transition can return the **outgoing** avatar's id and be indistinguishable from a correct answer. Adopting it would index the wrong table and swap somewhere the wearer never asked for, silently. Requiring two reads a configured gap apart to agree makes that window inert instead, so a press during it does nothing. The gap is **provisional**: nothing yet measures how long a stale value can be served, and a live probe is what should set it.

**The `/avatar/change` handler is registered ungated, and that is load-bearing.** It is both the only trigger that re-reads the marker and the only path out of a deactivated state, so gating it makes deactivation terminal — one 404 would silence the wardrobe until the process restarted. Worse, since a *successful* swap is what triggers the read, the gated version fails precisely on working, while an ineligible id leaves it healthy.
