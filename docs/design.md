...
**The logged stream is the change-filtered stream, and one event fires each callback once.** One row per *transition*: a repeated identical value never lands (`_update_cache_and_fire` suppresses it), which also folds the doubled inbound delivery (§Inbound delivery semantics) into one row; and `_on_osc_event` dedupes callback objects across exact and pattern registrations, so an overlapping whitelist (`SyncProbe/*` plus `SyncProbe/Reset`) is still one row per change rather than a doubled rate. A consumer needing every datagram — measuring the doubling itself, say — cannot use this tool, by design; the open measurement above keeps its own harness.
