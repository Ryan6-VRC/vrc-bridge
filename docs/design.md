...
**Why a marker parameter rather than one configured list.** The slot→avatar table cannot live on the avatar (they are a user's own avatar ids) and the bridge cannot otherwise know which table a given avatar's menu means. So each manifest carries an `id`, the avatar carries that number as the *default value* of an unsynced Int parameter, and the bridge reads it over OSCQuery. That is what lets two avatars carry different wardrobes, which is the whole justification — do not add "so we can tell a wardrobe-less avatar apart", because an avatar without the menu never sends a non-zero slot and zero is ignored anyway. Ids are 1–255, which is not our choice: **Modular Avatar's inspector clamps an Int default to 0–255 and re-clamps whenever the sync type is touched**, so a larger id cannot be authored onto the avatar that would have to carry it. That bounds any id convention built on this mechanism — worth knowing before designing another one around a wider range.

**The marker is read on the first press after an avatar change, never on the change itself.** This is the design's load-bearing choice and it removes a whole timing model. The press is the readiness signal: a Slot datagram can only arrive if the new avatar is loaded and emitting, so a marker read at that instant necessarily describes the avatar being worn. Reading on the change instead means reading *during* a transition, where `osc.md`'s only guarantee — the tree 404s an address *no worn avatar declares* — does not help, because this mapping's premise is that several avatars declare this one at different values, so the outgoing avatar's id can come back looking correct.

**No window could have been sized for that, which is why the scheduled design was abandoned rather than tuned.** The client acknowledges an avatar change immediately (below), while a cold avatar download runs 30–60 s; a scheduled read therefore polls an avatar that does not exist yet, and an earlier version concluded "this avatar has no wardrobe" on a ~6.5 s budget while the avatar was still downloading — leaving a wardrobed avatar unarmed until the *next* change, reachable only through the menu this feature exists to replace. Lazy arming has no budget to exceed: a press at t+60 s reads the marker at t+60 s. Deleted with the schedule: the settling delay, the stability gap, the read count, the two-agreeing-reads rule, the generation counter, and the worker thread. **Do not reintroduce a schedule here.**
...
**Which is only safe because arming and enablement are separate.** `_active` is the arm state and the mapping never writes its own `enabled`; that belongs to the router. The ungated handler therefore only ever *invalidates* — a mapping that could promote itself is the one thing an ungated handler must not be, and an earlier version did exactly that by calling `activate()` when it adopted a manifest, re-enabling a wardrobe a router had switched off. No other mapping in this repo calls its own `activate()`.
