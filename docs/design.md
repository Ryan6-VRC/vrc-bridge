...
**No echo watchdog, deliberately.** Detecting "the swap did not take" by watching for the return echo is the obvious next step and is absent on purpose: the wearer's currently-worn avatar is normally one of the eight slots, so pressing that slot legitimately produces no echo and the watchdog would fire on the most ordinary press, training its reader to ignore it. It also needs a window sized against measured client behaviour — including what the client does with an *ineligible* id, which no fake can answer.

**Worn-avatar suppression is a courtesy, not the loop defence.** The defence is that the source is momentary and zero never swaps. Suppression only skips a send the client would no-op anyway, and arms on the first observed change.
