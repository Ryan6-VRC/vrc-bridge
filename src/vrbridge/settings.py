...
class VirtualLensSettings:
    ...
    The three ranges below match VirtualLens2's shipped defaults, and each encoder
    in index_virtuallens is that product's own inverse rather than a fit to it. But
    they remain *per-install* settings on VL2's side: an avatar whose author widened
    the focal or f-number range needs these changed to match. VL2 publishes its own
    configured values as `VirtualLens2_Zoom_Min` / `_Zoom_Max` / `_Aperture_Min` /
    `_Aperture_Max` / `_Exposure_Range`, which is a cheaper check than reading a
    prefab -- and, unread, the reason a mismatch stays silent."""
    focal_min_mm: float = 12.0
    focal_max_mm: float = 300.0
    fnumber_min: float = 1.0
    fnumber_max: float = 22.0
    exposure_range_ev: float = 3.0
    aperture_min_x: float = 0.0001       # separates the f-max rung from the x==0 DoF-off rung
    # No press_duration here, and none is needed: VL2 fires on the transition into a
    # Control value and zeroes the parameter itself, so the latched write in
    # index_virtuallens.toggle_drop is correct. Settled, not deferred.
